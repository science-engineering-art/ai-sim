...
class A_star:
    ...
    def real_distance(ctrl, init_road_id, next_road_id, end_road_id):
    ...
    def find_shortest_path(ctrl, init_road_id : Road, end_road_id : Road, g_increment = real_distance, h = euclidean_distance):
        ...
        while len(queue) > 0:
            ...
            for next_road_id in road.end_conn.follow[road_id]:
                increment = g_increment(ctrl, init_road_id, next_road_id, end_road_id)
                next_road = ctrl.roads[next_road_id]
                if f[next_road_id] <= d + increment + h(next_road.end, end_road.start):
                    continue
                p[next_road_id] = road_id
                f[next_road_id] = d + increment + h(next_road.end, end_road.start)
